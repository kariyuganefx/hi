...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
	...
	if (new Date - chat.add < timeout) throw `_This command is still on cooldown!_\nplease wait for *${conn.msToDate(time - new Date())}* again!`
    ...
    for (let jid of response) {
        ...
        if (__user) {
            conn.sendButton(m.chat, `Added successfully @${jid.split`@`[0]}`, wm, 'menu', usedPrefix + 'menu', m)
        }
        if (!__user) {
            conn.sendButton(m.chat, `Failed to add @${jid.split`@`[0]}!\nMaybe the user just left this group so he had to enter through *${usedPrefix}link*`, wm, 'link', usedPrefix + 'link', m)
            /*
