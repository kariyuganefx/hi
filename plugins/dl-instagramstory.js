...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `uhm.. username?\n\nexample:\n\n${usedPrefix + command} toxic_turbo777`
    if (args[0].startsWith('http') || args[0].startsWith('@')) throw `enter username\n\nexample: *${usedPrefix}${command} toxic_turbo777*`
    try {
    await m.reply(wait)
...
handler.help = ['story'].map(v => v + ' <username>')
