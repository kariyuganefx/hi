let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (new Date - user.lastweekly > 604800000) {
        conn.send2Button(m.chat, `congrats you got *Rp.${md} ${rpg.emoticon('money')}money dan ${lg} ${rpg.emoticon('legendary')}Legendary crate*`, wm, 'menu', usedPrefix + 'menu', 'monthly', usedPrefix + 'monthly', m)
        user.money += md * 1
        user.legendary += lg * 1
...
function button(teks, user) {
    ...
    if (claim) buttons.push({buttonId: `.claim2`, buttonText: {displayText: 'CLAIM'}, type: 1})
