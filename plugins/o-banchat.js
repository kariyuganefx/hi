let handler = async (m, { conn, isOwner, text, isAdmin }) => {
  ...
  try {
    ...
    conn.sendButtonDoc(m.chat, `Succeed Ban! ${await conn.user.name} not active in chat ${await conn.getName(who) == undefined ? 'ini' : await conn.getName(who)}.`, wm, 'Owner', '.owner', m)
  } catch (e) {
