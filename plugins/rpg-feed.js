let handler = async (m, { conn, args, usedPrefix }) => {
    ...
    switch (type) {
        case 'rubah':
            if (rubah == 0) return m.reply('*You dont have a Pet Fox yet*')
            if (rubah == 5) return m.reply('*Your pet is max lvl*')
            ...
            } else m.reply(`Your pet is full, try feeding it *${waktur}* again`)
            break
        case 'kuda':
            if (kuda == 0) return m.reply('*You dont have a Pet Horse yet*')
            if (kuda == 5) return m.reply('*Your pet is max lvl*')
            ...
            } else m.reply(`Your pet is full, try feeding it *${waktuk}* lagia`)
            break
        case 'kucing':
            if (kucing == 0) return m.reply('*You dont have a Pet Cat yet*')
            if (kucing == 5) return m.reply('*Your pet is max lvl*')
            ...
            } else m.reply(`Your pet is full, try feeding it *${waktul}* again`)
            break
        case 'anjing':
            if (anjing == 0) return m.reply('*You dont have a Pet Dog yet*')
            if (anjing == 5) return m.reply('*Your pet is max lvl*')
                ...
                if (global.db.data.users[m.sender].makananpet > 0) {
                    ...
                    if (anjing > 0) { 
                        ...
                        if (global.db.data.users[m.sender].anakanjing > naiklvl) {
                            ...
                            conn.reply(m.chat, `*Congratulations your Pet Dog leveled up*`, m)
                        }
                    }
                } else m.reply(`Your pet food is not enough `)
            } else m.reply(`Your pet is full, try feeding it *${waktu}* again`)
            break    
            
        default:
            return conn.reply(m.chat, `${usedPrefix}feed [dog | cat | fox | horse]\nExamples of useq: *${usedPrefix}feed cat*`, m)
    }
}
