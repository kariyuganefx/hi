let handler = async (m, { conn, isOwner, text, isAdmin }) => {
  ...
  try {
    ...
    conn.sendButtonDoc(m.chat, `Successful unban! ${await conn.user.name} active chat ${await conn.getName(who) == undefined ? 'this' : await conn.getName(who)}.`, wm, 'Owner', '.owner', m)
  } catch (e) {
