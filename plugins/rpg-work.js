...
let handler = async (m, { conn, usedPrefix }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].healt > 79) {
            ...
            let str = `
${rpg.emoticon('healt')} Your life is reduced -${healt * 1} because you have adventured to ${pickRandom(['🇯🇵 Jepang', '🇰🇷 Korea', '🇮🇳 India', '🇺🇲 Amerika', '🇵🇸 Palestin', '🇮🇶 Iraq', '🇸🇦 Arab', '🇵🇰 Pakistan', '🇩🇪 German', '🇫🇮 Finlandia', 'To the world of dreams 😱', 'End of the world 🌏', 'Mars 👽', 'Moon 🌚', 'Pluto 😱', 'Sun 🌞', 'Her heart ♥️', '...'])} and get
${rpg.emoticon('exp')} *exp:* ${exp} 
${rpg.emoticon('money')} *uan:* ${uang}
${rpg.emoticon('sampah')} *rubbish:* ${sampah}${potion == 0 ? '' : `\n*${rpg.emoticon('potion')}Potion:* ` + potion + ''}${iron == 0 ? '' : `\n*${rpg.emoticon('iron')}Iron:* ` + iron + ''}${kayu == 0 ? '' : `\n*${rpg.emoticon('kayu')}Wood:* ` + kayu + ''}${batu == 0 ? '' : `\n*${rpg.emoticon('batu')}Rock:* ` + batu + ''}${string == 0 ? '' : `\n*${rpg.emoticon('string')}String:* ` + string + ''}${diamond == 0 ? '' : `\n*${rpg.emoticon('diamond')}diamond:* ` + diamond + ''}${common == 0 ? '' : `\n*${rpg.emoticon('common')}common crate:* ` + common + ''}${uncommon == 0 ? '' : `\n*${rpg.emoticon('uncommon')}uncommon crate:* ` + uncommon + ''}
`.trim()
            ...
            } else conn.sendButton(m.chat, `Youve worked hard today, please wait until *${timers}*`, wm, 'menu', usedPrefix + 'menu', m)
        } else conn.send2Button(m.chat, 'Minimum 80 health to be able to grow crops, buy medicine first to be strong by typing *' + usedPrefix + 'shop buy potion <amount>*\nand type *' + usedPrefix + 'use potion <amount>*\n\n_To get free money and potions when_ *' + usedPrefix + 'claim*', wm, 'Healing', usedPrefix + `heal`, 'Buy Potions', usedPrefix + `shop buy potion`, m)
    } catch (e) {
        console.log(e)
        throw eror
    }
